...
def run_learning_curve_fixed_test(
    target="Pi1",
    N_list=(50, 100, 200, 500, 800),  # ✅ IMPORTANT: max = 800 (train size)
    big_N=1000,                       # ✅ conforme à data_gen.py
    ridge_alpha=1.0,
    rf_params=None,
    seed_data=123,
    seed_split=42
    ...
    # ✅ on filtre N_list pour éviter l'erreur "larger sample"
    N_list = [N for N in N_list if N <= train_size]

    rng = np.random.default_rng(seed_split)
    rows = []

    for N in N_list:
        idx = rng.choice(train_size, size=N, replace=False)
...
if __name__ == "__main__":
    ...
    # Pi1
    df_pi1 = run_learning_curve_fixed_test(
        target="Pi1",
        N_list=(50, 100, 200, 500, 800),
        big_N=1000,
        ridge_alpha=1.0,
        rf_params=rf_params
    )
    ...
    # P
    df_p = run_learning_curve_fixed_test(
        target="P",
        N_list=(50, 100, 200, 500, 800),
        big_N=1000,
        ridge_alpha=100.0,  # (comme ton tuning l'avait suggéré)
        rf_params=rf_params
    )
    print("\n=== SCORES P ===")
    print(df_p)
